...
with st.sidebar:
    ...
    st.info("⚠️ 注意：回測起點受限於『最新上市』的 ETF (如 AVGS/AVGE)。")
...
try:
    ...
    if not df.empty:
        ...
        # --- 顯示 ---
        st.caption(f"起始資金: NT$ 1,000,000 | 目前匯率: {fx.iloc[-1]:.2f}")
        ...
        if stats_list:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "波動度 (Vol)": st.column_config.NumberColumn("年化波動度 %", format="%.2f %%"),
                    "夏普值 (Sharpe)": st.column_config.NumberColumn("夏普值 (CP值)", format="%.2f")
                },
